...
test("parseQaVerdict rejects malformed verdicts and fix tasks", () => {
  ...
  const { id: _, ...withoutId } = task("Q101")
  assert.deepEqual(fail([task("T003"), withoutId]).tasks.map((entry) => entry.id), ["Q101", "Q102"], "the harness numbers fix tasks")
  assert.deepEqual(fail([task("Q101")], 2).tasks.map((entry) => entry.id), ["Q201"])
  ...
  assert.deepEqual(fail([task("Q101"), task("Q101")]).tasks.map((entry) => entry.id), ["Q101", "Q102"], "duplicate ids are renumbered")
  assert.deepEqual(validateFixTasks([task("T001")], 1, existing).map((entry) => entry.id), ["Q101"], "existing ids are renumbered")
})
