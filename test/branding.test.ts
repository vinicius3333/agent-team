...
test("the old mockups key and gate map to branding", () => {
  ...
  assert.deepEqual(config.branding, { enabled: false, count: 5, mobile: true, variations: 3, dark: true, style: "auto" })
...
test("the branding key wins over mockups and defaults to 4 images", () => {
  ...
  assert.deepEqual(config.branding, { enabled: true, count: 4, mobile: true, variations: 3, dark: true, style: "auto" })
...
test("validateConcepts needs every direction with its logo, landing, and style", () => {
  ...
  for (const id of ["a", "b"]) {
    ...
    for (const file of ["logo.png", "landing.png"]) writeFileSync(join(conceptsDir, id, file), "")
  }
  writeFileSync(join(conceptsDir, "a", "style.md"), "Style: editorial-serif\n")
  writeFileSync(join(conceptsDir, "b", "style.md"), "- **Style:** `bento-grid`\n")
  ...
  writeFileSync(join(conceptsDir, "c", "style.md"), "Colors: warm\n")
  assert.throws(() => validateConcepts(dir, 3), /c\/style\.md must start with a "Style: <id>" line/)
  writeFileSync(join(conceptsDir, "c", "style.md"), "Style: bento-grid\n")
  assert.throws(() => validateConcepts(dir, 3), /directions b and c both use "bento-grid"/)
  assert.throws(() => validateConcepts(dir, 3, "neo-brutalism"), /a\/style\.md names "editorial-serif", but the person chose "neo-brutalism"/)
  writeFileSync(join(conceptsDir, "c", "style.md"), "Style: made-up\n")
  assert.throws(() => validateConcepts(dir, 3), /unknown style "made-up"/)
  writeFileSync(join(conceptsDir, "c", "style.md"), "Style: swiss-grid\n")
