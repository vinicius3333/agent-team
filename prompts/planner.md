...
3a. Install every package in the task that owns the package manifest, usually `T001`. Before you write the plan, list each package that any task's acceptance criteria, the architecture, or the design system names (for example `clsx`, `tailwind-merge`, `zod`), and put that list in the owning task's acceptance criteria. A task whose manifest is outside its `allowedPaths` cannot install anything, so it must depend on the task that installs what it uses.
3b. The app shell task wires every screen in `docs/design.md` into the app entry point and router before any feature task runs. For each screen it creates a placeholder component at the path the feature task will own (for example `src/pages/home.tsx` for `/`), renders it at its route, and lists those paths in its acceptance criteria. The feature task then fills in that file and never needs the entry point or the router. A single-page app mounts its sections the same way. Without this, a feature builds a component that nothing renders, and its screen stays blank.
...
6c. When `design/illustrations/` exists, the task that builds a screen with an illustration gets a `copy` entry, for example `"copy": [{"from": "design/illustrations/hero.png", "to": "public/illustrations/hero.png"}]`. The orchestrator copies the file before the worker starts, because workers may only write inside `allowedPaths`. The target must be inside `allowedPaths`. The acceptance criteria say to render the file with an image element. Never ask a worker to draw an illustration as SVG or CSS, or to copy files itself. When a screen needs art that `design/illustrations/` does not have, add an `illustrations` entry (`to`, a full image `prompt`, and a `reference` branding image); the orchestrator has the illustrator draw it before the worker starts.
...
6d. When `docs/architecture.md` has a `## Secrets` section, the task that adds each integration owns `.env.example` in its `allowedPaths` and adds the variable with its comment. When two tasks add integrations, the later one depends on the earlier, so they never share the file in parallel.
