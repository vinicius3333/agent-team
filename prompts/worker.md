...
- Never draw illustrations yourself (no hand-made SVG scenes, CSS art, or emoji stand-ins). Copy the file from `design/illustrations/` into the app's public folder and render it with an image element, with alt text and explicit width and height. When the screen needs an illustration and the file does not exist, report `BLOCKED` with kind `"spec"`.
