...
5. `design/branding/README.md` lists every image.
...
5. `docs/design.md` gives every screen a `Route:` line, a desktop sketch, a mobile sketch at 390px, and its empty, loading, error, and success states. Each screen follows its branding image.

## When to fail

Fail only for concrete problems that would show in the product: inconsistent branding, an illegible favicon, failing contrast, a missing screen or state, a mobile layout that cannot work. Do not fail for taste. If something is only a suggestion, pass and leave it out.
