...
- Third-party credentials (an OAuth client for Google login, payment, email, or AI API keys) come from environment variables that a person enters on the dashboard. List each one in `.env.example` at the repo root as a `NAME=` line with no value. Put a comment right above it that says what it is, where to get it, and what the provider needs from us, for example the OAuth redirect URI written as `<APP_URL>/api/auth/callback/google`. The dashboard shows the comment and fills in the live URL. Start the comment with `optional:` when the app works fully without it. Smoke checks and QA run without these values, so the app must start without them and use a clearly labeled fake until they are set (a demo sign-in button, a fake checkout). Deploy waits until each required one is entered or skipped. Describe them in `docs/architecture.md` under `## Secrets`.
