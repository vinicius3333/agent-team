...
1. `design/tokens.css`: the theme as shadcn/ui CSS variables for Tailwind CSS v4. Use the shadcn variable names (`--background`, `--foreground`, `--card`, `--primary`, `--secondary`, `--muted`, `--accent`, `--destructive`, `--border`, `--input`, `--ring`, `--radius`, `--chart-1` to `--chart-5`, `--sidebar-*`), plus `--success` and `--warning` with their `-foreground` pairs. Take the colors from the branding images. Give light values in `:root` and dark values in `.dark`. Take the dark values from the dark branding image (`02-landing.dark.png`) and the dark hex values in the README Style section when they exist; otherwise derive them from the light palette. Map every variable to a Tailwind color in an `@theme inline` block, so workers write `bg-primary` or `text-muted-foreground`, never raw hex values. Workers paste this file into the app's global stylesheet.
