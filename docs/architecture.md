...
- **Public URL:** the dashboard makes no absolute links of its own. `deploy.json` appends the host name from `APP_URL` to `AGENT_TEAM_UI_HOSTS` so requests to the public host are not refused. It adds the host even when `AGENT_TEAM_UI_HOSTS` is already set, so it is not only a default. The list stays comma-separated with no empty entries, and works when either value is empty or unset. Any other unknown host still gets 403.
