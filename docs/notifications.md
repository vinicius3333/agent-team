...
- Does the doctor's GitHub issue stay the only channel for incidents, or do incidents also go to these channels? This spec sends them; that may double up.
- Is `dashboardUrl` enough, or should links carry a login token? The dashboard has no login today and must not be exposed, so links only work on the tailnet.
- Should `stopped` (Ctrl+C or service restart) be sent? A deploy restarts runs, which could send noise.
- Is running the loop in both `ui` and `doctor` wanted, or should it be `doctor` only? `doctor` only is simpler, but then users without the doctor get nothing.
- Per-project overrides in `pipeline.yaml` (for example, mute one project)? Left out; `projects:` per channel covers the common case.
- Email adds the first optional dependency. Is `nodemailer` acceptable, or should email go through a webhook to an external mailer instead?
