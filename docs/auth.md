...
- `docker-compose.yml`, service `ui`: pass `AGENT_TEAM_UI_PASSWORD_HASH`, `AGENT_TEAM_UI_SESSION_SECRET`, `AGENT_TEAM_UI_TRUSTED_PROXIES`, and `AGENT_TEAM_UI_PROXY_USER_HEADER` through from the environment. Better: load them from `$HOME/.config/agent-team/doctor.env`, which the entrypoint already sources, so the hash and secret stay out of Dokploy. Note that `$` in the hash does not need escaping there, but does in a compose `environment:` value (`$$`). Prefer the file.
